...
import { MetricsAccumulator, PILOT_PROFILE, assertPilotFixtureContract, assertStagingOrigin, isSuccessfulAuthenticatedHtml, parseCookieFixture, parsePositiveInteger, parseSessionFixture, profileMinimums, qualificationOutcome } from "../scripts/pilot-load-lib.mjs";
...
test("requires meaningful authenticated app HTML", () => {
  expect(isSuccessfulAuthenticatedHtml(200, "text/html", '<html><main id="main-content">Queue</main></html>')).toBe(true);
...
test("uses bounded aggregate metrics and threshold outcomes", () => {
  ...
  expect(qualificationOutcome(summary)).toMatchObject({ passed: false, reasons: ["p95_limit"] });
  expect(qualificationOutcome({ ...summary, p95Ms: 100, errorRate: 0.01 })).toMatchObject({ passed: false, reasons: ["error_rate_limit"] });
});
...
test("pilot qualification accepts complete route and session coverage", () => {
  ...
  expect(qualificationOutcome(summary, undefined, PILOT_PROFILE, expectedSessions)).toMatchObject({ passed: true, profile: "pilot" });
});

test("labeled pilot sessions require fixture provenance and ten workspaces", () => {
  const origin = "https://staging.nudgepay.example";
  const workspaces = Array.from({ length: 10 }, (_, index) => ({ workspace: `pilot-workspace-${String(index + 1).padStart(2, "0")}`, invoices: 4_999, cases: 5_000, messages: 5_001 }));
  const sessions = Array.from({ length: 50 }, (_, index) => ({ cookie: `session=${index + 1}`, session: `pilot-session-${String(index + 1).padStart(2, "0")}`, workspace: `pilot-workspace-${String(Math.floor(index / 5) + 1).padStart(2, "0")}` }));
  const fixture = parseSessionFixture(JSON.stringify({ format: "nudgepay-pilot-session-fixture/v1", localOnly: false, source: { kind: "staging-pilot-fixture", origin, prefix: "pilot-load-20260905", totals: { organizations: 10, users: 50, invoices: 49990, cases: 50000, messages: 50010 }, workspaces }, sessions }));
  ...
  expect(() => assertPilotFixtureContract(invalidWorkspaceCountFixture, origin)).toThrow(/4,999/);
