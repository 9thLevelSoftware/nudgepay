...
describe("buildPersonalDataExport", () => {
  it("passes through account, membership, prefs, and logs", () => {
    const payload = buildPersonalDataExport({
      ...
      notificationPrefs: { brokenPromiseEmail: true, dailyDigestEmail: false },
      contactLogs: [
    ...
    expect(payload.membership?.orgName).toBe("Acme");
    expect(payload.notificationPrefs?.dailyDigestEmail).toBe(false);
