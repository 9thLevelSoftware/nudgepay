...
import { STANDING_LABEL } from "./AccountsDirectory";
  ...
  if (!account) {
    return (
      <aside className="hidden lg:flex flex-col items-center justify-center bg-surface border border-border rounded-card p-8 text-center text-muted">
        <Icon name="user" size={28} className="mb-2 text-muted" aria-hidden />
  ...
  return (
    <aside className="flex flex-col bg-surface border border-border rounded-card overflow-hidden">
      <header className="bg-ink text-surface px-4 py-3">
        ...
        <span className="mt-1 inline-flex items-center rounded-full bg-surface/10 px-2 py-0.5 text-xs font-medium text-on-ink">
      ...
      <div className="flex flex-col gap-2 p-4 border-t border-border">
        <Link
          ...
          className="inline-flex items-center gap-1.5 h-9 px-3 rounded bg-copper text-on-copper text-sm font-medium hover:bg-copper/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-copper self-start"
        >
