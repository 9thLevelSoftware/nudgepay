// Pure computation functions for the collections worklist — client-safe (no I/O,
// no server-only imports). Mirrors the exported functions from worklist.server.ts
// so that buildDashboardData in the route can be exported (and tested) without
// triggering React Router's server-only module bundler check.
//
// When worklist.server.ts is used server-side (loader), it calls these same
// functions. Both files must stay in sync; worklist.server.ts re-exports from
// here for DRY-ness is intentionally NOT done to keep the bundler boundary clean.
